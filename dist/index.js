...
async function callOtoboApi(operation, payload, retries = MAX_RETRIES) {
...
// 1. TicketCreate
server.tool("TicketCreate", "Create a new ticket in OTOBO. IMPORTANT: Before calling this tool, use ListStates to get valid states, ListPriorities to get valid priorities, and CustomerUserSearch to find/validate the customer user." + VALID_VALUES_INSTRUCTION, {
...
// 2. TicketGet
server.tool("TicketGet", "Get details of a specific ticket.", {
...
// 3. TicketHistoryGet
server.tool("TicketHistoryGet", "Get the history of a ticket.", {
...
// 4. TicketSearch
server.tool("TicketSearch", "Search for tickets based on various criteria. Use ListQueues and ListStates to get valid filter values." + VALID_VALUES_INSTRUCTION, {
    ...
    if (customerUser)
        payload.CustomerUserLogin = customerUser;
...
// 5. TicketUpdate
server.tool("TicketUpdate", "Update an existing ticket. IMPORTANT: Before calling this tool, use ListQueues, ListStates, ListPriorities to get valid values for the respective fields." + VALID_VALUES_INSTRUCTION, {
    ...
    if (queue)
        ticketData.Queue = queue;
    if (state)
        ticketData.State = state;
    if (priority)
...
// 6. TicketOverview
server.tool("TicketOverview", "Get a high-level overview of OPEN/active tickets (excludes closed or not required), optimized for LLM consumption. Use this for quick checks or general summaries.", {
    ...
    if (customerName)
        searchPayload.CustomerID = customerName;
    let searchResult;
    try {
...
server.tool("CustomerUserSearch", "Search for customer users.", {
...
// 8. CustomerUserGet
server.tool("CustomerUserGet", "Get details of a customer user.", {
...
server.tool("GetTicketMetadata", "DEPRECATED: Use ListQueues, ListStates, ListPriorities, ListTypes, ListAgents for live API data. This returns static config file values which may be outdated.", {}, async () => {
    try {
        if (fs.existsSync(configPath)) {
            const configContent = fs.readFileSync(configPath, 'utf-8');
...
// ============================================================================
// 10. ListQueues
server.tool("ListQueues", "Get all available queues from OTOBO. Use this to discover valid queue names before creating or updating tickets.", {
...
// 11. ListStates
server.tool("ListStates", "Get all available ticket states from OTOBO. Use this to discover valid state names before creating or updating tickets. States include type info (e.g., 'open', 'closed', 'pending').", {
...
// 12. ListPriorities
server.tool("ListPriorities", "Get all available ticket priorities from OTOBO. Use this to discover valid priority names before creating or updating tickets.", {
...
// 13. ListTypes
server.tool("ListTypes", "Get all available ticket types from OTOBO. Use this to discover valid type names before creating tickets.", {
...
// 14. ListAgents
server.tool("ListAgents", "Get all available agents/users from OTOBO. Use this to discover valid owner/responsible names before assigning tickets.", {
...
server.tool("TicketGetByNumber", "Get ticket details by ticket NUMBER (e.g., '2025010100001'). This is a convenience tool that first searches for the ticket ID, then retrieves full details. Use this when you have a ticket number instead of internal ID.", {
